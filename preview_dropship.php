...
if (isset($_SESSION['id']) && isset($_SESSION['user_name'])) {
  ...
  $namaDrop=$_GET['k'];
  $dataDrop=mysqli_query($conn,"SELECT * FROM customer WHERE ID=$namaDrop");


  $namaPemesan=$_GET['l'];
  $dataPemesan=mysqli_query($conn,"SELECT * FROM customer WHERE ID=$namaPemesan");

  $last=mysqli_query($conn,"SELECT Expedisi FROM transaksi WHERE ID_Cus=$namaPemesan");
...
<body>
  ...
  <thead>
    <tr>
      <th scope="col">Data Dropshiper</th>
      <th scope="col"></th>

    </tr>
  </thead>
  <tbody>
    <?php foreach ($dataDrop as $newData) :
      // code...
    ?>
    <tr>
      <th scope="row">Nama</th>
      <?php echo('<td>');?>: <?php echo ($newData['Nama']);?> </td>
    ...
    <tr>
      ...
      <td>: Pelanggan <?php echo ($newData['Golongan']); endforeach;?> </td>
  ...
  <thead>
    <tr>
      <th scope="col">Data Pemesan</th>
      <th scope="col"></th>

    </tr>
  </thead>
  <tbody>
    <?php foreach ($dataPemesan as $newData) :
      // code...
    ?>
    <tr>
      <th scope="row">Nama</th>
...
<div class="card">
  ...
  <button class="btn btn-primary" onclick="myFunction()" value="<?php echo $_GET['k'];?>" id="btnPemesan" hidden><i class="bi bi-check-circle"></i> Konfirmasi</button>
  <button class="btn btn-primary" onclick="myFunction()" value="<?php echo $_GET['l'];?>" id="btnDrop" hidden><i class="bi bi-check-circle"></i> Konfirmasi</button>
...
document.getElementById("lbl").innerHTML = Exp;
  ...
  function bukaWindow() {
  var idDrop=document.getElementById("btnDrop").value;
  var idP=document.getElementById("btnPemesan").value;
  var codeOpt =document.getElementById("code").value;
  var exped = document.getElementById("lbl").textContent;
  window.open("simpanTransaksi.php?k="+idDrop+"&l="+idP+"&exp="+encodeURIComponent(exped)+"&code="+codeOpt+"&drop=1");
}
</script>
